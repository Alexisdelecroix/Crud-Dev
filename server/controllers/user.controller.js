...
const getOneUsers = (req, res) => {
  const userId =  req.params.id;
    const query = `SELECT nom, prenom, adresse, ville, code_postal, telephone, email FROM users WHERE id = ${userId} `;
    conn.query(query, (err, result)=> {
     if (err) {
         console.error("Erreur lors de la récupération des données :" + err);
         res.status(500).json({error : "Erreur lors de la récupération des données"})
 ...
 const updateUser = (req, res) => {
    ...
    const query= 'UPDATE users SET nom = ?, prenom = ?, adresse = ?, ville = ?, code_postal = ?, telephone = ?, email = ? WHERE id = ?' ;
    
    conn.query(query, [nom, prenom, adresse, ville, code_postal, telephone, email, req.params.id], (err) => {
        if (err) {
...
module.exports = { 
    createUser, getAllUsers, updateUser, deleteUser, getOneUsers
}
